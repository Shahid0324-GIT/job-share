...
from app.schemas import BatchInput, FriendInput, JobInput, StatusInput
from app.security import hash_token, make_session, new_token, normalize_url, token_matches, valid_session
...
def add_job(payload: JobInput, request: Request, db: Session = Depends(get_db)):
    admin_required(request)
    ...
    job = Job(url=url, source=payload.source or extracted.source, company=payload.company or extracted.company, role=payload.role or extracted.role, location=payload.location or extracted.location, description=payload.description or extracted.description)
    db.add(job)
    db.commit()
    db.refresh(job)
    return {"job": job.id, "warnings": extracted.warnings}


@app.patch("/api/admin/jobs/{job_id}")
def edit_job(job_id: str, payload: JobInput, request: Request, db: Session = Depends(get_db)):
    admin_required(request)
    csrf_required(request)
...
def add_friend(payload: FriendInput, request: Request, db: Session = Depends(get_db)):
    ...
    token = new_token()
    friend = Friend(name=payload.name.strip(), token_hash=hash_token(token))
...
def regenerate_friend(friend_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    token = new_token(); friend.token_hash = hash_token(token); friend.active = True
    db.commit()
    return {"token": token}
...
def add_batch(payload: BatchInput, request: Request, db: Session = Depends(get_db)):
    admin_required(request); csrf_required(request)
    jobs = db.scalars(select(Job).where(Job.id.in_(payload.job_ids))).all()
...
def friend_page(request: Request, db: Session = Depends(get_db)):
    ...
    jobs = db.scalars(select(Job).join(BatchJob).join(Batch).where(BatchJob.job_id == Job.id)).unique().all()
...
def update_status(job_id: str, payload: StatusInput, request: Request, db: Session = Depends(get_db)):
    friend = public_friend(request, db)
    job = db.scalar(select(Job).join(BatchJob).where(Job.id == job_id))
